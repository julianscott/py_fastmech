...
def fastmech_change_var_cd2(hdf_file, newCd_Cd0,newCd_Cd1):
    file = h5py.File(hdf_file, 'r+')
    group = file['/iRIC/iRICZone/GridConditions/roughness/Value']
    dset = group[u' data']
    for value in list([0.0,1.0]):
        if value in dset:
            print("")
        else:
...
def check_Cd_classes(hdf_file,Cd0_class,Cd1_class):
    file = h5py.File(hdf_file, 'r+')
    group = file['/iRIC/iRICZone/GridConditions/roughness/Value']
    dset = group[u' data']
    for value in list([Cd0_class,Cd1_class]):
        if value in dset:
...
numcds = np.array(np.meshgrid(a0,a1)).T.reshape(-1,n_cd_classes)
...
check_Cd_classes(hdf5_file_name,Cd0_class,Cd1_class)
...
for cdind in range(numcds.shape[0]):
    ...
    fastmech_change_var_cd2(hdf5_file_name, newCd_Cd0 = Cd_Cd0,newCd_Cd1 = Cd_Cd1) 
...
#        cd_val_Cd2[cdind] = Cd_Cd2
        ...
        meod_val[cdind] = meod
        # For the given fastmech run, combine (sort of equivalent to R cbind) Cd class values, rmse, and mean error on discharge. 
        trmse = np.column_stack((cd_val_Cd0.flatten(),cd_val_Cd1.flatten(),rmse_data.flatten(),meod_val.flatten()))
...
df = pd.DataFrame(trmse,columns = ['Cd_Cd0','Cd_Cd1','rmse','meon'])
...
hdf5_file_name = "FM_Calib_" + str(Q) + "_0Cd" + str(Cd_Cd0) + "_1Cd" + str(Cd_Cd1) + "_.CGNS"
