...
# Set the config file 
setfile =  "E:\\_DoD\\_Camp_Pendleton_Survey\\IRIC\\_Modeling_dir\\_LowFlows_Model_v2\\Python_Directory\\q400_0\\q400_0cms_config.ini"
...
# set Lateral Eddy Viscosity settings
LEV_Type = config.getfloat('Params','LEV_Type')
LEV_Constant = config.getfloat('Params','LEV_Constant')
LEVStart_Iter = config.getfloat('Params','LEVStart_Iter')
LEVEnd_Iter = config.getfloat('Params','LEVEnd_Iter')
StartLEV = config.getfloat('Params','StartLEV')
EndLEV = config.getfloat('Params','EndLEV')
...
def execute(cmd):
    popen = subprocess.Popen(cmd, stdout=subprocess.PIPE, universal_newlines=True,stdin=subprocess.DEVNULL,stderr=subprocess.STDOUT)
...
# For changing roughness for nodes set to 0,1, 5 and 6 (i.e. vary 4 class of roughness nodes)  
def fastmech_change_var_cd2(hdf_file, newCd_Cd0,newCd_Cd1,newCd_Cd2,newCd_Cd3,newCd_Cd4):
    file = h5py.File(hdf_file, 'r+')
    group = file['/iRIC/iRICZone/GridConditions/roughness/Value']
    dset = group[u' data']
    for value in list([0.0,1.0,2.0,3.0,4.0]):
        if value in dset:
            print("")
        else:
    ...
    for index, val in enumerate(dset):
        if val == 0.0:
            ...
            if val == 1.0:
                ...
                if val == 2.0:
                    dset[index] = newCd_Cd2 # boulder/cobble cd class
                else:
                    if val == 3.0:
                        dset[index] = newCd_Cd3 # veg cd class 
                    else:
                            if val == 4.0:
                                dset[index] = newCd_Cd4 #  cd class 
    file.close()

def check_Cd_classes(hdf_file,Cd0_class,Cd1_class,Cd2_class,Cd3_class,Cd4_class):
    file = h5py.File(hdf_file, 'r+')
    group = file['/iRIC/iRICZone/GridConditions/roughness/Value']
    dset = group[u' data']
    for value in list([Cd0_class,Cd1_class,Cd2_class,Cd3_class,Cd4_class]):
        if value in dset:
...
def fastmech_params2(hdf_file):
    if LEV_Type == 0:
#    def fastmech_params(hdf_file, Itermax,LEV_Type,LEV_Constant):
        file = h5py.File(hdf_file, 'r+')
        group = file['/iRIC/CalculationConditions/FM_HydAttLEVType/Value']
        dset2 = group[u' data']
        dset2[0] = LEV_Type
        group = file['/iRIC/CalculationConditions/FM_HydAttEVC/Value']
        dset1 = group[u' data']
        dset1[0] = LEV_Constant
        group = file['/iRIC/CalculationConditions/FM_SolAttItm/Value']
        dset = group[u' data']
        dset[0] = sol_iterations
        file.close()    
    else:
        if LEV_Type == 1:
#        def fastmech_params(hdf_file, sol_iterations,LEV_Type,LEVStart_Iter,LEVEnd_Iter,StartLEV,EndLEV):
            file = h5py.File(hdf_file, 'r+')
            group = file['/iRIC/CalculationConditions/FM_HydAttLEVType/Value']
            dset1 = group[u' data']
            dset1[0] = LEV_Type
            
            group = file['/iRIC/CalculationConditions/FM_HydAttLEVStartIter/Value']
            dset2 = group[u' data']
            dset2[0] = LEVStart_Iter
            
            group = file['/iRIC/CalculationConditions/FM_HydAttLEVEndIter/Value']
            dset3 = group[u' data']
            dset3[0] = LEVEnd_Iter
            
            group = file['/iRIC/CalculationConditions/FM_StartLEV/Value']
            dset4 = group[u' data']
            dset4[0] = StartLEV
            
            group = file['/iRIC/CalculationConditions/EndLEV/Value']
            dset5 = group[u' data']
            dset5[0] = EndLEV
            
            group = file['/iRIC/CalculationConditions/FM_SolAttItm/Value']
            dset6 = group[u' data']
            dset6[0] = sol_iterations
...
# Cd2 = boulder/cobble nodes
cdmin_Cd2 = eval(config.get('Params','cdmin_Cd2'))
cdmax_Cd2 = eval(config.get('Params','cdmax_Cd2'))
cdn_Cd2 = eval(config.get('Params','cdn_Cd2'))
# Cd3 = veg noes
cdmin_Cd3 = eval(config.get('Params','cdmin_Cd3'))
cdmax_Cd3 = eval(config.get('Params','cdmax_Cd3'))
cdn_Cd3 = eval(config.get('Params','cdn_Cd3'))
# Cd4 = veg noes
cdmin_Cd4 = eval(config.get('Params','cdmin_Cd4'))
cdmax_Cd4 = eval(config.get('Params','cdmax_Cd4'))
cdn_Cd4 = eval(config.get('Params','cdn_Cd4'))
...
# This code conditionally defines the function meodfun, depending on whether LEV is constant or variable.
if LEV_Type == 1:
    def meodfun(path):
        return re.search('(.+?)\n', path).group(1)
#        return re.search('(.+?)\n', path).group(1)
...
a2 = np.linspace(cdmin_Cd2,cdmax_Cd2,int(cdn_Cd2))
a3 = np.linspace(cdmin_Cd3,cdmax_Cd3,int(cdn_Cd3))
a4 = np.linspace(cdmin_Cd4,cdmax_Cd4,int(cdn_Cd4))
...
numcds = np.array(np.meshgrid(a0,a1,a2,a3,a4)).T.reshape(-1,n_cd_classes)
...
cd_val_Cd2 = np.zeros(numcds.shape[0])
cd_val_Cd3 = np.zeros(numcds.shape[0])
cd_val_Cd4 = np.zeros(numcds.shape[0])

# Check to make sure that the Cd classes (0,1, 5 and 6) are present in the initial Case1.cgn file.
...
check_Cd_classes(hdf5_file_name,Cd0_class,Cd1_class,Cd2_class,Cd3_class,Cd4_class)
...
for cdind in range(numcds.shape[0]):
    ...
    Cd_Cd2 = numcds[cdind,2]
    Cd_Cd3 = numcds[cdind,3]
    Cd_Cd4 = numcds[cdind,4]
    # For the given fastmech run, populate containers to track Cd value by class
    cd_val_Cd0[cdind] = Cd_Cd0
    cd_val_Cd1[cdind] = Cd_Cd1
    cd_val_Cd2[cdind] = Cd_Cd2
    cd_val_Cd3[cdind] = Cd_Cd3
    cd_val_Cd4[cdind] = Cd_Cd4
    ...
    fastmech_params2(hdf5_file_name)

    # Reclassify grid nodes for Cd roughness classes
    fastmech_change_var_cd2(hdf5_file_name, newCd_Cd0 = Cd_Cd0,newCd_Cd1 = Cd_Cd1,newCd_Cd2 = Cd_Cd2,newCd_Cd3 = Cd_Cd3,newCd_Cd4 = Cd_Cd4) 
...
#        When fastmech solution is not found, populate result boxes with -9999
        ...
        trmse = np.column_stack((cd_val_Cd0.flatten(),cd_val_Cd1.flatten(),cd_val_Cd2.flatten(),rmse_data.flatten(),meod_val.flatten()))
    else:
#        Else, when there is a Fastmech solution, create VTK grid and get results
        ...
        print("Cd_boulder/cobble:",Cd_Cd2)
        print("Cd_veg:",Cd_Cd3)
        print("Cd_other:",Cd_Cd4)
...
#        cd_val_Cd2[cdind] = Cd_Cd2
        ...
        meod_val[cdind] = meod
        # For the given fastmech run, combine (sort of equivalent to R cbind) Cd class values, rmse, and mean error on discharge. 
        trmse = np.column_stack((cd_val_Cd0.flatten(),cd_val_Cd1.flatten(),cd_val_Cd2.flatten(),cd_val_Cd3.flatten(),cd_val_Cd4.flatten(),rmse_data.flatten(),meod_val.flatten()))
...
df = pd.DataFrame(trmse,columns = ['Cd_Cd0','Cd_Cd1','Cd_Cd2','Cd_Cd3','Cd_Cd4','rmse','meon'])
...
Cd_Cd2 = best_mod['Cd_Cd2']
Cd_Cd3 = best_mod['Cd_Cd3']
Cd_Cd4 = best_mod['Cd_Cd4']
hdf5_file_name = "FM_Calib_" + str(Q) + "_0Cd" + str(Cd_Cd0) + "_1Cd" + str(Cd_Cd1) + "_2Cd" + str(Cd_Cd2) + "_3Cd" + str(Cd_Cd3) + "_4Cd" + str(Cd_Cd4) + "_.CGNS"
