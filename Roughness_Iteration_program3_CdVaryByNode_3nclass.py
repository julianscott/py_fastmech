...
# Tool is developed by Julian Scott, but is strongly dependent on code written by Rich McDonald. In general, the defined
# functions are primarily Rich (except def fastmech_change_var_cd2, check_Cd_classes, and fastmech_change_var_H_DS).
# The VTK grid code is Rich, as is the simulated WSE extraction by point code. Julian wrote the defined functions given above,
# and wrote the code to iterate over all combinations of multiple classes of roughness.  Most annotation is also by Julian.
...
# For changing constant roughness for nodes set to 0,1,and 5 (i.e. vary 3 class of roughness nodes)  
...
# Set the config file 
setfile =  "E:\\_DoD\\_Camp_Pendleton_Survey\\IRIC\\_Modeling_dir\\_LowFlows_Model_v2\\Python_Directory\\q1_00\\q1_0cms_config.ini"
...
# Cd0 = base nodes
cdmin_Cd0 = config.getfloat('Params','cdmin_Cd0')
cdmax_Cd0 = config.getfloat('Params','cdmax_Cd0')
cdn_Cd0 = config.getfloat('Params','cdn_Cd0')
# Cd1 = dogleg reach nodes
cdmin_Cd1 = config.getfloat('Params','cdmin_Cd1')
cdmax_Cd1 = config.getfloat('Params','cdmax_Cd1')
cdn_Cd1 = config.getfloat('Params','cdn_Cd1')
# Cd5 = boulder/cobble nodes
cdmin_Cd5 = config.getfloat('Params','cdmin_Cd5')
cdmax_Cd5 = config.getfloat('Params','cdmax_Cd5')
cdn_Cd5 = config.getfloat('Params','cdn_Cd5')

# This is from Rich's original code; I believe it allows you to retrieve a node value 
# from a given coordinate offset by the amount given here. Leave set to 0.
...
for cdind in range(numcds.shape[0]):
    ...
    # Update solution iterations; the second argument is not used as long as LEV is constant (Currently inherited and unmodified from Case1.cgn). 
    fastmech_params(hdf5_file_name, 1000, 0.036)
...
copyfile(config.get('Params','base_file'),
         hdf5_file_name)
fastmech_BCs(hdf5_file_name, Q, H_DS, iniType, OneDCD, Cddistribution)
fastmech_params(hdf5_file_name, 1500, 0.036)
fastmech_change_var_cd2(hdf5_file_name, newCd_Cd0 = Cd_Cd0,newCd_Cd1 = Cd_Cd1,newCd_Cd5 = Cd_Cd5)
for path in execute(["Fastmech.exe", hdf5_file_name]):
    meod = path
    try:
